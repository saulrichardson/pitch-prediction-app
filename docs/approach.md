...
The web role has no model invocation permission. The retired timeline-start
jobs, self-invoking worker, per-click inference, job polling, and mutable
...
- `scripts/lib/preparation.ts` owns inference leases, resumability, attempt
...
- Model normalization stays in `services/model-api/`; MLB ingestion stays in
  `mlb-service.ts` and the domain normalizer.
...
The current publication workflow is operator-driven. It has no new scheduler or
standing service. A failed new edition leaves the last good edition available;
...
seconds; no 300-second public background worker remains. AWS budget alerts are
