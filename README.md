...
The [delivery runbook](docs/records/2026-09-12-prepared-replay-delivery.md) records
the prepared-replay cutover. The [game browser record](docs/records/2026-09-12-game-browser.md)
describes the additive stream worker, catalog indexing, and rollout checks.
After verification, integrate and push `main`, deploy, smoke-check the live app,
and remove completed work branches. Delivery is part of the task.
...
- `packages/workflows/`: schedule discovery, durable game preparation, resumable
  inference, lease, budget, and publication.
...
Public routes are `/api/replays`, `/api/replays/[id]`, `/api/games`, and
