...
export function preparationCallerId(request: Request): string {
  const viewerIp = request.headers.get("x-viewer-ip");
  if (!viewerIp && process.env.NODE_ENV === "production")
    throw new Error("Trusted CloudFront viewer identity is missing.");
  return pseudonymizeViewerIp(viewerIp ?? "127.0.0.1", sessionSecret());
}
...
function verifySessionToken(
  token: string,
): { workspaceId: string; createdAt: number } | null {
  ...
  try {
    ...
    ) as { workspaceId?: string; createdAt?: number };
    if (!parsed.workspaceId || typeof parsed.createdAt !== "number")
      return null;
...
function sessionSecret(): string {
  ...
  if (!secret) {
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "SESSION_SECRET or APP_SECRET_JSON.sessionSecret is required in production.",
      );
    }
  ...
  if (
    ...
    (secret.length < 32 || knownUnsafeSecrets.has(secret))
  ) {
    throw new Error(
      "SESSION_SECRET must be a non-default secret with at least 32 characters in production.",
    );
  }
